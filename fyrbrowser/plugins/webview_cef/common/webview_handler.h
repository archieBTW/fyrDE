...
public:
    ...
    //download event
    std::function<void(int browserId, std::string suggestedName, std::string url)> onDownloadStart;
    std::function<void(int browserId, std::string url, int64_t receivedBytes, int64_t totalBytes, int percent, bool isComplete)> onDownloadUpdated;
    
    //context menu event
    std::function<void(int browserId, int x, int y, int typeFlags, std::string linkUrl, std::string sourceUrl, std::string selectionText, bool isEditable)> onContextMenu;
    ...
    void continueFileDialog(int callbackId, const std::vector<std::string>& filePaths);
