...
void gtk_window_iconify(void *window) {
  ...
  if (!real_iconify)
    real_iconify = dlsym(RTLD_NEXT, "gtk_window_iconify");
  if (real_iconify)
    real_iconify(window);
  do_minimize();
}
...
void gtk_window_minimize(void *window) {
  ...
  if (!real_min)
    real_min = dlsym(RTLD_NEXT, "gtk_window_minimize");
  if (real_min)
    real_min(window);
  do_minimize();
}

void window_manager_minimize() { do_minimize(); }

void gtk_actionable_set_action_name(void *actionable, const char *action_name) {
  static void (*real_set)(void *, const char *) = NULL;
  static unsigned long (*signal_connect)(void *, const char *, void *, void *,
                                         void *, int) = NULL;
  ...
  if (real_set)
    real_set(actionable, action_name);
}
...
void gtk_widget_set_sensitive(void *widget, int sensitive) {
  ...
  if (!sensitive && has_css_class && widget) {
    if (has_css_class(widget, "minimize") ||
        has_css_class(widget, "maximize")) {
      sensitive = 1;
    }
  }

  if (real_set)
    real_set(widget, sensitive);
}
