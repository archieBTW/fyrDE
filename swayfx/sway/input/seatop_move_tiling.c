...
struct seatop_move_tiling_event {
	...
	struct wlr_scene_rect *indicator_blur;
};
...
static void update_indicator(struct seatop_move_tiling_event *e, struct wlr_box *box) {
	...
	wlr_scene_rect_set_size(e->indicator_blur, box->width, box->height);
	...
	wlr_scene_rect_set_corner_radius(e->indicator_blur, corner_radius_real, CORNER_LOCATION_ALL);
}
...
void seatop_begin_move_tiling_threshold(struct sway_seat *seat,
	...
	e->indicator_blur = wlr_scene_rect_create(seat->scene_tree, 0, 0, (float[4]){0.0, 0.0, 0.0, 0.0});
	if (!e->indicator_blur) {
		free(e);
		return;
	}
