...
void root_scratchpad_add_container(struct sway_container *con, struct sway_workspace *ws) {
	...
	con->scratchpad_was_floating = container_is_floating(con);

	// When a tiled window is sent to scratchpad, center and resize it.
	if (!config->scratchpad_minimize && !container_is_floating(con)) {
...
void root_scratchpad_show(struct sway_container *con) {
	...
	if (config->scratchpad_minimize && !con->scratchpad_was_floating) {
	...
	if (new_ws->output && container_is_floating(con)) {
