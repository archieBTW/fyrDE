...
void AudioController::open_mixer()
{
    ...
    // Try common mixer names
    const char* names[] = {"Master", "PCM", "Digital", "Headphone", "Speaker"};
    snd_mixer_elem_t *elem = nullptr;
    for (const char* name : names) {
        elem = get_mixer_element(name);
        if (elem) {
            std::cout << "Found mixer element: " << name << std::endl;
            break;
        }
    }
    ...
    {
        std::cerr << "Failed to find any compatible mixer element." << std::endl;
        // Don't throw here, let the plugin handle a null controller if needed,
        // or just have a non-functional volume control instead of a crash.
        // But the plugin expects it to be created.
    }
}
